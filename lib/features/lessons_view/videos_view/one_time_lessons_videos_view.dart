...
class OneTimeLessonsVideosView extends StatefulWidget {
  final int videoIndex;
  const OneTimeLessonsVideosView({super.key, required this.videoIndex});

  @override
  _OneTimeLessonsVideosViewState createState() => _OneTimeLessonsVideosViewState();
}

class _OneTimeLessonsVideosViewState extends State<OneTimeLessonsVideosView> {
  ...
  void showLoadOnRefresh() async {
    isLoad = true;
    await LessonsCubit.get(context).getVideosByClassesWithCode(
      code: LessonsCubit.get(context).codeController.text,
      classID:LessonsCubit.get(context).oneTimeLessonData['class_data']['id']
    );
    Future.delayed(Duration(seconds: 2)).then((value) {
      ...
      Navigator.push(
        ...
        CupertinoPageRoute(
          builder:
              (context) => OneTimeLessonsVideosView(
            videoIndex: LessonsCubit.get(context).oneTimeLessonData['id'],
          ),
        ),
      );
      isLoad = false;
  ...
  void initState() {
    ...
    CoursesCubit.get(context).isCourseLoading = false;
     ...
     LessonsCubit.get(context).getVideosByClassesWithCode(
        code: LessonsCubit.get(context).codeController.text,
        classID:LessonsCubit.get(context).oneTimeLessonData['id']
    );
  ...
  void _loadVideos() {
    final data = LessonsCubit.get(context).oneTimeLessonData;
    if (data == null) return;
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<LessonsCubit, LessonsState>(
      listener: (context, state) {
        state is GetVideosByClassesWithCodeDone;
      },
            ...
            return isLoad
              ...
              body: RefreshIndicator(
                ...
                child: Directionality(
                  ...
                  child: SafeArea(
                    child:
                    LessonsCubit.get(context).oneTimeLessonData == null &&
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  ...
                                  spacing: 5,
                                  children: [
                                    LessonsCubit.get(context).oneTimeLessonData['attachment'] != null
                                        ? _buildActionButton(
                                      context,
                                      Icons
                                      ...
                                      'تحميل المذكرة',
                                          () async {
                                        var url = LessonsCubit.get(context).oneTimeLessonData['attachment'];
                                    ...
                                    LessonsCubit.get(
                                      context,
                                    ).oneTimeLessonData['has_homeworks'] ==
                                        true
                                        ? _buildActionButton(
                                      context,
                                      Icons.assignment,
