                    ...
                    child: MaterialButton(
                      onPressed: () async {
                        if (formKey.currentState!.validate()) {
                          ...
                          await cubit.sendOTP(email: emailController.text, context: context);

                          if (cubit.remainingSeconds != null && cubit.otpVia != null) {
                            Navigator.push(
                              context,
                              CupertinoPageRoute(
                                builder: (_) => SubmitOtpScreen(
                                  email: emailController.text,
                                  remainingSeconds: cubit.remainingSeconds!,
                                ),
                              ),
                            );
                          }
                      ...
                      child: Text('ارسال', style: TextStyles.textStyle16w700(context).copyWith(color: Colors.white)),
                    ),
                  ),
                ],
              ),
